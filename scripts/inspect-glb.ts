...
async function main(): Promise<void> {
  const { file, root, requiredNames, requiredClips, json, nodes } = parseArguments();
  ...
  const missingClips = requiredClips.filter(name => !summary.animations.some(animation => animation.name === name));
...
function parseArguments() {
  const { values, positionals } = parseArgs({
    ...
    options: {
      root: { type: 'string' },
      require: { type: 'string' },
      'require-clip': { type: 'string' },
      json: { type: 'boolean', default: false },
      nodes: { type: 'boolean', default: false },
    },
  });
  if (positionals.length !== 1) {
    throw new Error('Usage: node --import tsx scripts/inspect-glb.ts <file.glb> [--root <name>] [--require <name,name,...>] [--require-clip <name,name,...>] [--nodes] [--json]');
  }
  return {
    file: positionals[0],
    root: values.root,
    requiredNames: values.require?.split(',').map(name => name.trim()).filter(Boolean) ?? [],
    requiredClips: values['require-clip']?.split(',').map(name => name.trim()).filter(Boolean) ?? [],
    json: values.json,
    nodes: values.nodes,
  };
}
...
function formatSummary(summary: GlbSummary, includeNodes: boolean): string {
  ...
  const lines = [
    ...
    ...summary.animations.map(animation => `  ${animation.name}: ${animation.duration}s → ${animation.targetNodes.join(', ')}`),
